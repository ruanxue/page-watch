...
function QbittorrentSettingsPage({ onNotice }: { onNotice: (message: string) => void }) {
  ...
  return <><section id="downloads" className="qbit-settings-page">
    <div className="section-head"><div><h2>qBittorrent 下载</h2></div></div>
    <form className="qbit-settings" onSubmit={submit}>
      <section className="qbit-card"><div className="qbit-card-head"><div><h3>连接设置</h3></div><label className="toggle"><input type="checkbox" checked={form.enabled} disabled={loading || busy} onChange={(event) => update('enabled', event.target.checked)} /><span />启用下载</label></div>
