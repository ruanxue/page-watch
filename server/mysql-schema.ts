...
export async function ensureMySqlSchema(pool: Pool) {
  ...
  // Do not use MySQL triggers for this read model. Many NAS MySQL accounts
  // deliberately run with binary logging and without SUPER, which makes
  // CREATE TRIGGER fail even though the account can create the application
  // tables. Progress is reconciled by the application after startup and on
  // its normal maintenance pass, so deployment never needs elevated MySQL
  // privileges or log_bin_trust_function_creators.
